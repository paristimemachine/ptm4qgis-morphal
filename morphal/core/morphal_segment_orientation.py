...
class MorphALSegmentOrientation(PTM4QgisAlgorithm):
    ...
    def __init__(self):
        ...
        self.units = [self.tr('Degree'),
                      self.tr('Radian'),
                      self.tr('Grade')]
        self.intervals = [self.tr('[0 ; Pi['),
                          self.tr('[0 ; Pi/2[')]
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                ...
                minValue=0.001, maxValue=200, defaultValue=10
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                ...
                type=QgsProcessingParameterNumber.Double,
                minValue=0.001, maxValue=200, defaultValue=5
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.OUTPUT,
                self.tr('Segments with orientation')
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        classification = self.parameterAsBoolean(parameters, self.CLASSIFICATION, context)
        classification_step = self.parameterAsDouble(parameters, self.CLASSIFICATION_STEP, context)

        histogram = self.parameterAsBoolean(parameters, self.HISTOGRAM, context)
        histogram_step = self.parameterAsDouble(parameters, self.HISTOGRAM_STEP, context)
        ...
        if method == 1:
            ...
            coord_transform = QgsCoordinateTransform(source.sourceCrs(), context.project().crs(), context.project())
