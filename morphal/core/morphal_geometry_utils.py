...
def polygon_elongation(polygon: QgsPolygon):
    ...
    '''
    # if len(polygon.vertices()) != 5:
    if _geometry_num_vertices(polygon) != 5:
        return -1.0

    vertices = _geometry_vertices(polygon, 2)

    # v0 = QgsPoint(vertices[0])
    # v1 = QgsPoint(vertices[1])
    # v2 = QgsPoint(vertices[2])
    ...
    return lentgh2 / lentgh


def is_circle(polygon: QgsPolygon, i_miller_threshold: float,
...
def is_rectangle(polygon: QgsPolygon, sd_convex_hull_threshold: float,
                 sd_mbr_threshold: float, distance_area: QgsDistanceArea):
    '''
